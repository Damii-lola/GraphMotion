...
## 8. One world, six shots (added after the "it is a slideshow" review)

Six unrelated pictures joined by dives read as a slideshow, however good the transition. The references are ONE scene filmed by ONE camera. `backend/kleinClient.js` uses FLUX.2 [klein] on Workers AI, which both generates and **edits** pictures: shot 1 is generated from text (or is the client's own photo), and every later shot is an *edit of the previous shot* ("the camera pushes in on the sketch; its lines glow"), so the place, objects and light stay identical while camera, action and story beat change. The AI writes a director's shot list instead of six independent picture prompts (with a worked example in the prompt because the small model drifts without one). At 512x1024 (2 x 512-px tiles) a film costs about 430 neurons of pictures (~72 per edit) versus ~550 for six FLUX schnell pictures; 768x1344 is 6 tiles (~3x). If klein is unavailable before anything is made, the old six-separate-pictures path is used (`IMAGE_ENGINE=flux` forces it).

## 9. Movie mode (real motion instead of stills + dives)

Stills joined by transitions - even stills of one world - are still a slideshow. Movie mode makes the ad one continuous generated FILM: shot 1 is a picture (FLUX.2 klein, ~240 neurons, optionally an edit of the company's own share picture), then each scene is an image-to-video clip (`backend/videoGen.js`, LTX-Video distilled via a Hugging Face Space) that starts on the LAST frame of the previous clip and follows that scene's director's note. The clips are slowed to exactly 3.5 s each and joined into `movie.webm` (VP9, 512x1024, what headless Chrome can decode). `ad.html` then has no WebGL engine and no transitions: the page just seeks the video to the exact frame for every captured frame (deterministic) and lays the captions on top. Requires `HF_TOKEN` (a free token gives more GPU quota than anonymous; each film needs ~100 GPU-seconds). If the video model is unavailable before the first clip exists, the old stills engine takes over; if a later clip fails, the last picture is held with a slow push-in. `MOVIE=0` disables it.
