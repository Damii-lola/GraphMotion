 ...
 * specifically on a 429 - real, direct production evidence this matters:
 * "[imagePrefetch] beat 2 image failed, falling back to procedural:
 * Pollinations returned 429", and a real generated video confirmed
 * missing its hero photo as a direct result (the "src":"beatImage"
 * layer was correctly paired with a real "imagePrompt", it just had
 * nothing to draw once the fetch failed). The ORIGINAL single retry
 * fired IMMEDIATELY with no delay - for a 429 specifically (the server
 * explicitly saying "you're going too fast"), retrying at the exact
 * same instant is close to useless, it hits the same rate-limit window
 * again. A short, real backoff (1.5s, then 3s) gives the free,
 * unauthenticated endpoint's own rate-limit window an actual chance to
 * clear before trying again. A non-429 failure (timeout, malformed
 * response) still gets exactly one immediate retry, same as before -
 * no reason to slow those down with a backoff a rate limit doesn't
 * apply to.
 */
