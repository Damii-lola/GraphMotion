...
- "fontFamily" MUST be EXACTLY one of those eight strings - no other font name (not "Poppins Regular", not a real commercial typeface) is bundled and will silently fall back to an unstyled default.
- TWO real typeface families exist now, each with its own mood - pick per beat, don't default to the same one every time: Poppins (Black/Bold/Medium/Italic) is a bold, punchy, modern grotesque - the safe default for most beats, especially anything energetic, urgent, or Gen-Z. Playfair Display (Black/Bold/Regular/Italic) is an elegant, editorial SERIF - genuinely, visibly different in character, not just a style tweak - reach for it on a beat that wants to feel premium, reflective, or a little more grown-up (a luxury topic, a quieter/emotional line, a beat that's meant to feel like a real statement rather than a shout). Playfair Display Italic in particular reads as a soft, considered aside - good for a single quieter beat inside an otherwise punchy Poppins video, not for every beat in a row. Never mix two DIFFERENT font families within the same single text layer (not possible anyway - "fontFamily" is one value per layer) - the variety comes from choosing differently BEAT TO BEAT, not within one line.
...
- YOUR BEAT'S DOMINANT TEXT LAYER SHOULD BE THIS BEAT'S OWN "narration" LINE, WORD FOR WORD - direct requirement, not a suggestion: this engine measures the REAL audio timing of the narration once it's spoken and reveals THAT SAME on-screen text one word at a time in perfect sync with the actual voice (word appears exactly as it's said, like a karaoke lyric) - this only works when the words match. A dominant text layer with different wording than its own beat's narration cannot be synced this way.
- ONE text layer per beat, ONLY - direct user correction (real screenshots): a second caption-style text layer sitting under the main headline ("Oxygenrich flow", "Musclefuelled circulation", "Hemocyanin is thick") is NOT wanted, ever, even short. The narration line IS the beat's only on-screen text - do not add a second label, tagline, or fact underneath it.
...
function buildCompactGenerationSystemPrompt(chosenTemplates, explicitItemCount = null) {
  ...
  const enumerationRule = explicitItemCount
    ? `\n- The user's request explicitly names ${explicitItemCount.count} ${explicitItemCount.noun} - your beats' combined narration MUST clearly name/describe EXACTLY ${explicitItemCount.count} distinct, concrete ${explicitItemCount.noun}, never more and never fewer (e.g. "First, ... Second, ... Third, ..." spread across beats, or one clearly-numbered item per beat) - never a vague summary that skips naming them individually. Decide your full list of ${explicitItemCount.count} up front, before writing any beat, so every beat draws from that SAME fixed list. If you have more beats than ${explicitItemCount.count}, the extra beats must build anticipation, add context, or ask a question about ONE OF THOSE SAME ${explicitItemCount.count} items - NEVER introduce a new item/example/habit/tip that isn't on your list of ${explicitItemCount.count}, and never number past "${explicitItemCount.count}". Confirmed real failure to NEVER imitate: a "3 habits ruining your sleep" video whose early beats mentioned "phone before bed," "stress," and "exercising too late" as separate problems, then a later recap beat announced a DIFFERENT set of "the 3 habits" - the viewer heard 6 different habits total for a video that promised 3. Pick your 3 first, mention only those 3 anywhere in the video.`
    : '';
...
- EVERY beat must support the SAME stated topic/premise - never let one beat's own content contradict it. Confirmed real failure to NEVER imitate: a video titled "habits that are RUINING your sleep" had one beat present "Consistent Sleep Schedule / Dark Bedroom / Avoid Caffeine" as if they were more bad habits - those are actually GOOD habits, the opposite of the topic, making the beat read like it was written for a different video. Before finalizing, check every beat against the topic: if the topic is negative (bad habits, red flags, mistakes, things ruining X), every beat's content must ALSO be negative/cautionary - never slip in a positive/solution-style beat, and vice versa.
