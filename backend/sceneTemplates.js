...
const SHARED_PARAMS = {
  narration: {
    ...
    maxLength: 70,
    default: '',
    description: 'OPTIONAL spoken voiceover line for this beat, generated as real speech audio and automatically timed - this beat\'s on-screen duration will stretch or shrink to match however long the narration actually takes to say (plus a short pad), so don\'t also hand-pick "duration" for a narrated beat, it will be overridden. THIS is where the full sentence/thought goes - narration can be a complete, natural sentence, unlike on-screen text. KEEP EACH LINE SHORT (well under the character max, not right up against it) - the camera holds mostly at rest for a beat\'s ENTIRE on-screen duration, so a long narration line directly means a long visually-static hold, which reads as dead air on fast-cut short-form video. A longer point is a reason to split it across TWO OR THREE beats (each its own short narration line, its own camera position/arrival) rather than one long line in one beat - more, shorter, narrated beats keep the camera moving and feel far more alive than one beat that talks for a long time while nothing changes on screen. Leave "" for a silent beat (keeps its own authored/default duration). Not every beat needs narration - a purely visual beat or a quick punctuation beat can stay silent.',
  },
  tag: {
    type: 'string',
    maxLength: 16,
  ...
  customShapeRecipe: {
    type: 'shapeRecipe',
    default: [],
    description: 'For heroVisual (or icon) content that has NO good match in the fixed lists above (a guitar, a pizza, a plant, an animal, literally anything) - build it yourself from safe geometric primitives instead of forcing it into a mismatched fixed shape. An array of up to 14 primitives, each one: {"type": "circle"|"rect"|"triangle"|"polygon"|"arc"|"line", "x": -1 to 1, "y": -1 to 1, plus type-specific fields}. circle/arc need "r" (0.03-1.2). rect needs "w","h" (and optional "rx" for rounded corners). triangle/polygon need "points": array of [x,y] pairs (3-8 points). arc needs "startAngle"/"endAngle" in degrees. line needs "x2","y2". All coordinates are relative to center (0,0), roughly -1 to 1 covering the icon\'s bounds. Every shape is automatically shaded (a highlight-to-shadow gradient across its own footprint) - you don\'t need to do anything for that. Two OPTIONAL fields add real depth on top: "depth": "front"|"mid"|"back" (default "mid") makes one shape read as nearer (brighter) or further (darker) than its neighbors - e.g. the lit front face of an object vs. a receded detail behind it. "opacity": 0.3-1 (default 1) for a soft/faint secondary shape (a shadow, a glow, a background element behind the main form). COMPOSITION TECHNIQUE (this is what separates a good result from a blob): use TWO overlapping circles of different sizes for anything with an organic waisted curve (a guitar body, a bottle, a vase) rather than one plain circle - a single circle reads as a ball, not the object. Vary "depth" across a recipe\'s shapes too - a recipe where every shape is "mid" looks flatter than one where the main form is "front" and a background/interior detail is "back". A worked example, a guitar: [{"type":"circle","x":0,"y":0.45,"r":0.4,"depth":"front"},{"type":"circle","x":0,"y":0.05,"r":0.28,"depth":"front"},{"type":"rect","x":0,"y":-0.5,"w":0.14,"h":0.85,"rx":0.04,"depth":"mid"},{"type":"circle","x":0,"y":0.3,"r":0.12,"fill":false,"depth":"back"}] - two circles for the waisted body (front, most prominent), a rect for the neck (mid), a hollow circle for the sound hole (back, reads as receding into the body). Only use this when heroVisual\'s fixed list genuinely has nothing close - prefer the fixed icons/marks when one actually fits, they render faster and more reliably.',
  },
  imagePrompt: {
    type: 'string',
    maxLength: 140,
    default: '',
    description: 'A short, concrete, photographic description for a REAL AI-generated photo to use as this scene\'s hero visual, replacing heroVisual/customShapeRecipe entirely for this beat. THIS SHOULD BE SET ON MOST BEATS IN MOST VIDEOS, not treated as an occasional accent - a real photo reads as dramatically more premium and closer to real short-form footage than a flat vector icon/glyph, and a video built entirely from vector icons is the single most common reason output looks generic and templated instead of close to real reference video. Default to reaching for a real photo; only skip it (leave "") for a beat that genuinely doesn\'t want one - a bare number/stat beat, a UI-widget beat (checklistTick, gridReveal, statGrid), or a beat on iconCallout/badgeUnlock (they always draw their own inline icon and don\'t support this at all). This applies to ABSTRACT and conceptual subjects too, not just literal physical objects - find a photographable mood, metaphor, or scene that evokes the idea rather than defaulting to an icon just because the topic isn\'t a literal object: "AI/technology/the future" -> "glowing blue neural network light patterns in dark space, futuristic" or "a hand touching a holographic interface, cinematic blue lighting"; "growth/progress" -> "a small green plant sprout breaking through dark soil, soft morning light"; "urgency/warning" -> "a red warning light glowing in a dark room, dramatic shadow". Write every prompt like a stock-photo brief: concrete subject + setting + lighting/mood (e.g. "a lit vintage brass lantern on a dark wooden table, warm cinematic lighting" or "a stack of hundred dollar bills on a marble counter, soft daylight"). NEVER ask for a "split image", "before and after", "left side X right side Y", or any prompt describing two different things in one photo - this free image model is unreliable at that specific composition and tends to render two similar/near-identical objects instead of genuine contrast (confirmed directly: asking for "left side a budgeting app, right side a spilled coffee cup" produced two near-identical coffee cups side by side, not a real comparison). One photo = one single coherent subject/scene, always. On a comparison-style beat (splitCompare, duoStatCompare, barChartCompare), either leave imagePrompt empty and let that beat\'s own two-column layout carry it, or request a photo for just ONE side\'s concept, not both. NEVER ask for text, words, letters, numbers, or logos to appear IN the image - image models render text as garbage, and this scene\'s own text already renders separately on top. Pairs especially well with the "visualMoment" template (a pure photographic beat, no competing icon). Image generation can fail or be slow (it\'s a best-effort free service) - the scene automatically falls back to its normal vector hero visual if so, so always still set heroVisual too as that fallback, even on a beat where you\'re confident imagePrompt will land.',
  },
};
...
const TEMPLATES = {
  kineticTextReveal: {
    description:
      'Bold statement text that punches in with scale overshoot and a glow pulse. THIS IS THE PRIMARY TEMPLATE for almost all text delivery, in three modes: (1) RUNS mode (PREFERRED - reach for this first) - set "textRuns" to a sequence of {text, size, color, highlight} pieces that together form the beat\'s statement, each piece independently sized/colored/highlighted, e.g. a sentence where one word explodes to huge size while the rest stays small - this is real typographic hierarchy within a single sentence, not a uniform block of text at one size. (2) single mode - set "text" to ONE short phrase/clause (2-6 words) at one uniform size, for when the whole statement should read as one visual weight. (3) GROUP mode - set "lines" (2-4 short related phrases) instead, to show several related short statements stacked together in ONE beat, each with its own full-size kinetic reveal - this is how to handle a "list" of related points (reasons, steps, related facts) with the same bold energy as a single statement, INSTEAD OF reaching for listReveal/checklistTick/gridReveal/statGrid/arrowFlow, which render as small utilitarian UI widgets - visually much weaker than kinetic typography and should be reserved for when the content is genuinely that specific UI metaphor. Exactly ONE of "textRuns", "text", or "lines" should be meaningfully set per beat.',
    params: {
      textRuns: { type: 'runArray', default: [], maxItems: 10, description: 'PREFERRED mode - an ordered sequence of text pieces that concatenate (with spaces) into the beat\'s full statement, each independently styled: {"text": string (a word or short phrase, max 30 chars), "size": "small"|"normal"|"large"|"huge" (default "normal"), "color": "primary"|"accent"|"muted" (default "primary"), "highlight": boolean (default false, draws a solid accent-color highlight box behind just this piece, like a marker stroke)}. Use ONE "huge" or "large" piece per statement as the dominant focal word/phrase - don\'t make everything the same size, that defeats the point. Worked example for "but if you keep making excuses": [{"text":"but","size":"small","color":"muted"},{"text":"if you","size":"small","color":"accent"},{"text":"KEEP","size":"huge","color":"primary","highlight":true},{"text":"making excuses","size":"normal","color":"primary"}]. Leave [] if using "text" or "lines" instead.' },
      text: { type: 'string', default: '', maxLength: 90, description: 'Single-statement mode (one uniform size/color for the whole phrase). Leave "" if using "textRuns" or "lines" instead.' },
      lines: { type: 'stringArray', default: [], maxItems: 4, maxItemLength: 40, description: 'GROUP mode - 2-4 short related phrases (e.g. ["Too rigid for real life", "Too many decisions", "No room for chaos", "Built for tracking, not humans"]) stacked and revealed together in this one beat, each at full kinetic size. Leave empty ([]) if using "textRuns" or "text" instead.' },
      style: { type: 'enum', values: ['bold-glow', 'mixed-weight'], default: 'bold-glow', description: 'Only affects single-statement ("text") mode - textRuns has its own per-piece size/color control instead.' },
      textFrame: { type: 'enum', values: ['none', 'card', 'gradient', 'highlight'], default: 'none', description: 'Visual framing: none (plain glow), card (a soft rounded card background), gradient (text fill sweeps white-to-accent, single mode only), highlight (an animated marker-style highlight sweeps behind each line/statement). Vary this across beats - don\'t use the same one every time.' },
      duration: { type: 'number', min: 0.7, max: 6, default: 1.6, description: 'Single mode: a short 2-6 word fragment should be 0.7-1.4s. GROUP mode ("lines"): budget roughly 0.9-1.3s PER LINE (e.g. 3 lines ~= 3-4s total) so each line actually gets time to land - don\'t give a 4-line group the same short duration as a single fragment.' },
      ...SHARED_PARAMS,
    },
  },
  ...
  iconCallout: {
    ...
    params: {
      ...
      duration: { type: 'number', min: 0.8, max: 3.5, default: 1.6 },
      ...SHARED_PARAMS,
    },
  },
...
function buildMistralSystemPrompt(targetDurationSeconds = 12) {
  ...
  // iconCallout fragments commonly run 0.7-1.5s each. 1.7s blends that
  // fast-cut majority with the genuinely longer templates (lists, grids,
  // quotes) that still need several seconds. Min/max scene count is
  // still a guide, not a hard instruction - Mistral should pace beats
  // by content, not pad to a number - but it should land much higher
  // for a video with a lot of dialogue/copy to deliver than the old
  // divisor implied.
  const approxScenes = Math.round(duration / 1.7);
  const minScenes = Math.max(2, Math.round(approxScenes * 0.75));
  // Capped at 40, not the validator's hard 42-scene limit - leaves a
  // little headroom so a slightly-over guess doesn't get hard-rejected.
...
- Default toward the SHORTER end of each template's duration range, not the middle or longer end - real fast-cut short-form video rarely holds a single beat for its full possible length. Reserve a longer duration within a beat's range for when that specific beat genuinely needs it (a multi-item list/grid needs enough time for every item's stagger to land; a quote needs a breath to read) - a single bold statement or stat does not. Note this only matters for beats WITHOUT narration - a narrated beat's actual on-screen duration is driven by the spoken audio length regardless of what "duration" is set to, so don't overthink duration on narrated beats.
- CRITICAL - NARRATION CARRIES THE STORY, ON-SCREEN TEXT DOES NOT. Real short-form video has a voiceover doing the actual talking; on-screen text is a sparse visual accent on top of that, not a transcript. This system can generate real spoken narration (the "narration" shared param, documented above) - USE IT on most beats with real content, written as natural, complete sentences (unlike on-screen text, narration is NOT limited to short fragments). Once narration is carrying a beat's meaning, its on-screen "text"/"lines" should be radically sparser than the narration itself - usually just the single most important word or a 2-4 word emphasis fragment pulled from what's being said, sometimes NO on-screen text at all (an image/icon beat can be narrated with zero text on screen). A beat that puts its full sentence on screen AND speaks the same full sentence is redundant and reads as cluttered - never do both at full length. Example: narration "Your daily coffee run is quietly draining your bank account" pairs with on-screen text as small as "Draining." or "$1,800/year" or nothing at all - not the whole sentence repeated as text.
- Text-grouping rules (apply to whatever on-screen text you DO show, now that it should generally be short): PREFER kineticTextReveal's "textRuns" for almost every statement - real hierarchy within the sentence (one word huge, the rest small/muted) reads as far stronger and more scroll-stopping than a whole phrase at one uniform size, and matches how the actual best-performing reference videos treat text. Fall back to plain "text" (one uniform size) only for a genuinely short fragment where hierarchy wouldn't add anything (2-3 words, all equally important). A GROUP of several related short fragments that belong together (when you genuinely want more than one short line on screen at once, which will be less common now that narration exists) is kineticTextReveal with "lines" (2-4 short items) instead - full bold kinetic-typography energy, not run into one sentence and not blown out into separate beats. Only reach for listReveal/checklistTick/gridReveal/statGrid/arrowFlow when the content is genuinely that specific UI metaphor (an actual completable checklist, an actual small grid of numbers) - their small checkbox/bullet/grid-card chrome is the exception, not the default way to show related text.
...
- CRITICAL: the FIRST beat of the video is what decides whether anyone keeps watching - a real scroll-stopping hook has to be on screen almost immediately, not after several seconds of a silent shape. If the prompt has an actual hook, claim, question, or statement to make (which is true for almost every prompt except the bare single-object case above), the FIRST beat MUST be a text-forward template ("kineticTextReveal" is usually right) that puts words on screen right away - never open on "visualMoment" or any other silent/text-free beat when there's real content to lead with. Save "visualMoment" for later in the video (a breather/payoff beat) or for the rare case where the entire prompt is just naming one object with nothing else to say.
...
- Pick ONE "visualSystem" for the WHOLE video (not per beat) from: "neonPulse" (near-black background, one saturated neon accent with real glow, a giant near-invisible echo of each beat's tag looming behind its content, a small signature mark holding the same screen position all video - the highest-energy, most "2026 short-form" system, DEFAULT TO THIS for hooks, warnings, callouts, urgency, tech, money, and most punchy/attention-grabbing content unless another system is clearly a better fit), "hudTerminal" (dark, glowing, data/HUD chrome - fits finance/data content that wants to feel more like a dashboard/insider-info readout than a raw hook), "softEditorial" (light, calm, serif, no glow/chrome - fits reflective, lifestyle, psychology, personal-essay tones), "boldGraphic" (flat saturated color blocks, high contrast, no glow - fits punchy hooks that want a poster/graphic-design feel rather than neonPulse's glow-heavy energy). Choose based on the PROMPT's tone, not a default. CRITICAL: if the prompt asks for energy, "dynamic", "punchy", fast, exciting, or anything in that family, that OVERRIDES a light/soft color mention - softEditorial is the calm, restrained system by design and will read as slow and lifeless no matter how it's animated, so it is the WRONG choice whenever real energy is requested, even if the prompt also asks for a light/pastel/cream palette. Use "neonPulse" or "boldGraphic" for that combination instead - "backgroundMood": "light" still gives either one a lighter background if truly needed (visualSystem and backgroundMood are independent choices), though neonPulse's whole identity is built around staying dark. Reserve softEditorial for prompts that are ACTUALLY calm/reflective in their own request, not just light-colored.
- When "visualSystem" is "neonPulse", also set "signatureMotif" (one of: "bracket", "crosshair", "dots", "arrow", "plus", "triangle") - a small mark that stays in the exact same screen corner for the WHOLE video, like a channel watermark. Pick one that fits the content's feel and is DIFFERENT from whatever any individual beat's own "accentShape" values are doing (accentShape varies per beat on purpose; signatureMotif must NOT vary - it is the one constant across the entire video). Ignored by every other visualSystem, so no need to set it otherwise.
...
- CRITICAL - "imagePrompt" (documented under shared params above) puts a REAL generated photo into a beat instead of a flat vector icon. This should be set on MOST beats in MOST videos - it is the default hero treatment now, not a rare accent. A video rendered as vector-icon-only has consistently been reported as looking generic, flat, and templated compared to real reference footage; real photography is the single biggest lever available for closing that gap within a $0 procedural budget. Only leave a beat on the vector fallback when it genuinely doesn't want a photo: a bare number/stat beat, a UI-widget beat (checklistTick/gridReveal/statGrid/arrowFlow), or iconCallout/badgeUnlock (unsupported, they draw their own icon). This includes abstract/conceptual topics, not just literal objects - find a photographable mood or metaphor for the idea (see the worked examples under the shared param itself) rather than falling back to an icon just because nothing literal comes to mind. Still set heroVisual on every beat regardless, as the automatic fallback for when a specific image fetch fails.
...
function validateSceneJSON(json) {
  ...
  const cleanScenes = json.scenes.map((scene, i) => {
    ...
    for (const [pname, pdef] of Object.entries(tmpl.params)) {
      ...
      if (pdef.type === 'runArray') {
        const SIZES = ['small', 'normal', 'large', 'huge'];
        const COLORS = ['primary', 'accent', 'muted'];
        if (!Array.isArray(val)) val = pdef.default || [];
        val = val
          .filter((item) => item && typeof item === 'object' && typeof item.text === 'string' && item.text.trim().length > 0)
          .map((item) => ({
            text: clampStringSmart(item.text, 30),
            size: SIZES.includes(item.size) ? item.size : 'normal',
            color: COLORS.includes(item.color) ? item.color : 'primary',
            highlight: item.highlight === true,
          }))
          .slice(0, pdef.maxItems || 10);
        if (val.length === 0 && pdef.required) {
          throw new Error(`scene[${i}] (${scene.template}) param "${pname}" needs at least one run`);
        }
      }
  ...
  // Only meaningful on systems with showSignatureMotif on (currently
  // neonPulse) - harmless if set and ignored by a system that doesn't
  // use it, so no cross-validation against visualSystem needed here.
  const VALID_MOTIFS = ['bracket', 'crosshair', 'dots', 'arrow', 'plus', 'triangle'];
  const signatureMotif = VALID_MOTIFS.includes(json.signatureMotif) ? json.signatureMotif : 'crosshair';
