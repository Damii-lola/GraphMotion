...
const SHARED_PARAMS = {
  ...
  imagePrompt: {
    ...
    maxLength: 140,
    default: '',
    description: 'OPTIONAL: a short, concrete, photographic description for a REAL AI-generated photo to use as this scene\'s hero visual, replacing heroVisual/customShapeRecipe entirely for this beat (leave "" to keep using the normal vector hero system, which is still the default and still perfectly good). Use this SELECTIVELY, not on every beat - a video that mixes a few real photographic hero-moments with procedural/text/data beats reads as far more dynamic than either all-vector or all-photo. Reach for it when the prompt centers on a literal, physical, photographable subject (a product, a specific real-world object or scene, a mood/place) where an actual photo would land harder than an icon. Write it like a stock-photo brief: concrete subject + setting + lighting/mood (e.g. "a lit vintage brass lantern on a dark wooden table, warm cinematic lighting" or "a stack of hundred dollar bills on a marble counter, soft daylight"). NEVER ask for text, words, letters, numbers, or logos to appear IN the image - image models render text as garbage, and this scene\'s own text already renders separately on top. Pairs especially well with the "visualMoment" template (a pure photographic beat, no competing icon). Not available on iconCallout/badgeUnlock, which always draw their own inline icon. Image generation can fail or be slow (it\'s a best-effort free service) - the scene automatically falls back to its normal vector hero visual if so, so never skip setting heroVisual just because you set imagePrompt.',
  },
};
...
- "imagePrompt" (documented under shared params above) puts a REAL generated photo into a beat instead of the vector hero system - use it on a handful of beats per video when the content centers on a literal photographable subject, not on every beat (a video that's all photos is as monotonous as one that's all icons) and never on iconCallout/badgeUnlock beats (they always show their own inline icon instead).
