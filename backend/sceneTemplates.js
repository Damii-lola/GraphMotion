...
const TEMPLATES = {
  kineticTextReveal: {
    description:
      'Bold statement text that punches in with scale overshoot and a glow pulse. THIS IS THE PRIMARY TEMPLATE for almost all text delivery, in two modes: (1) single mode - set "text" to ONE short phrase/clause (2-6 words), for a single standalone statement that deserves its own beat/camera-arrival. (2) GROUP mode - set "lines" (2-4 short related phrases) instead of "text", to show several related short statements stacked together in ONE beat/camera position, each with its own full-size kinetic reveal - this is how to handle a "list" of related points (reasons, steps, related facts) with the same bold energy as a single statement, INSTEAD OF reaching for listReveal/checklistTick/gridReveal/statGrid/arrowFlow, which render as small utilitarian UI widgets (checkboxes, bullet dots, thin-outline grid cards, numbered badges) - visually much weaker than kinetic typography and should be reserved for when the content is genuinely that specific UI metaphor (an actual completable checklist, an actual small grid of stat numbers), not used as the default way to group text. When in doubt, prefer "lines" grouping over both a full sentence in one beat AND over the small-widget list templates.',
    params: {
      text: { type: 'string', default: '', maxLength: 90, description: 'Single-statement mode. Leave "" if using "lines" instead.' },
      lines: { type: 'stringArray', default: [], maxItems: 4, maxItemLength: 40, description: 'GROUP mode - 2-4 short related phrases (e.g. ["Too rigid for real life", "Too many decisions", "No room for chaos", "Built for tracking, not humans"]) stacked and revealed together in this one beat, each at full kinetic size. Leave empty ([]) if using "text" instead. Exactly one of "text" or "lines" should be meaningfully set per beat, never both.' },
      style: { type: 'enum', values: ['bold-glow', 'mixed-weight'], default: 'bold-glow', description: 'Only affects single-statement ("text") mode.' },
...
- Text-grouping rules (apply to whatever on-screen text you DO show, now that it should generally be short): a SINGLE standalone short phrase is kineticTextReveal with "text". A GROUP of several related short fragments that belong together (when you genuinely want more than one short line on screen at once, which will be less common now that narration exists) is kineticTextReveal with "lines" (2-4 short items) instead of "text" - full bold kinetic-typography energy, not run into one sentence and not blown out into separate beats. Only reach for listReveal/checklistTick/gridReveal/statGrid/arrowFlow when the content is genuinely that specific UI metaphor (an actual completable checklist, an actual small grid of numbers) - their small checkbox/bullet/grid-card chrome is the exception, not the default way to show related text.
