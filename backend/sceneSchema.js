...
const { FONT_PAIRINGS } = require('./engine/fonts');
...
// Derived from FONT_PAIRINGS (engine/fonts.js), not hand-duplicated -
// that file is the single source of truth for which aliases are really
// registered; a Set dedupes the repeats every single-weight family
// (Bebas Neue, Archivo Black, Anton) creates by pointing all 4 of its
// own normalized keys at the same one real alias.
const AVAILABLE_FONT_FAMILIES = [...new Set(FONT_PAIRINGS.flatMap((p) => [...Object.values(p.sans), ...Object.values(p.serif)]))];
...
function validateBeatVisual(visual, path, errors, knownIds, opts = {}) {
  ...
  // Skipped entirely for mograph beats (opts.isMographBeat) - their own
  // visual.layers are 100% mechanically generated by a template builder
  // in this same file, never model-authored, so the "plain copy +
  // accented copy" mistake this check exists to catch is structurally
  // impossible there. A real, confirmed-live false positive found
  // building textTiers: a natural phrase can legitimately reuse a short
  // common word (e.g. "the") across two DIFFERENT lines, each its own
  // independently-positioned/animated per-word layer by design, not a
  // duplicate mistake - this check (and the matching auto-repair merge
  // a few hundred lines down) used to silently eat the second "the"
  // layer outright, a real, silent word-loss bug, not just a spurious
  // rejection.
  if (!opts.isMographBeat) {
...
function autoRepairBeat(beat) {
  ...
  if (Array.isArray(beat.visual.layers)) {
    ...
    // Skipped for mograph beats - see validateBeatVisual's own matching
    // opts.isMographBeat gate for the full reasoning: this beat's own
    // visual.layers are 100% mechanically generated, never model-
    // authored, so the "plain copy + accented copy" mistake this merge
    // exists to clean up can't happen here - a real, confirmed-live bug
    // found building textTiers, where this used to silently EAT a
    // legitimately-repeated common word ("the" reused across two
    // different lines of one natural phrase) as if it were that mistake.
...
function validateBeat(beat, path = 'beat') {
  ...
  validateBeatVisual(beat.visual, `${path}.visual`, errors, knownIds, { skipTextRequirement: isPlainObject(beat.mograph), isMographBeat: isPlainObject(beat.mograph) });
...
/**
 * Built from a real frame-by-frame read of a user-attached reference
 * video (A4.mp4): a short punchy statement builds word-by-word (a
 * "typewriter" pace, but per WORD not per letter) across 3 lines, each
 * word popping in blurred-and-small then sharpening + overshooting to
 * its final size. Direct user description of the 3-line hierarchy,
 * confirmed via real pixel measurement of the reference's own final
 * frame, not guessed: line 1 is the smallest ("setup" line); line 2 is
 * much bigger (the "escalation"); line 3 mixes both - its own FIRST
 * word matches line 1's small size, the REST of line 3 is a hair bigger
 * than line 2. Line 1 (+ line 3's small first word) render in the
 * pairing's SANS face, dim/muted except line 1's own first word; line 2
 * (+ the rest of line 3) render in the pairing's SERIF face, in the
 * beat's own accent color for line 2 and a bright neutral for line 3 -
 * matches the reference's own warm-gold-vs-cool-silver two-tone
 * treatment without hardcoding either literal color.
 *
 * Direct follow-up user ask: "add more fine fonts... randomize the font
 * FAMILIES, the scenes just pick the fonts inside the chosen family" -
 * one of FONT_PAIRINGS (engine/fonts.js) is picked deterministically per
 * beat (hashString(text), same seeding pattern this file already uses
 * for accentColor fallback) rather than via Math.random() - keeps a
 * given beat's own font choice stable across re-renders/re-validation.
 */
const TEXT_TIERS_SMALL_SIZE = 20;
const TEXT_TIERS_BIG_SIZE = 46;
// "the last line is a tad bit bigger than the middle line" - direct
// user spec.
const TEXT_TIERS_BIGGEST_SIZE = 51;
...
// but line 3 deliberately mixes two. Without compensation, two
// different-sized middle-anchored words drawn at the same Y end up
// vertically CENTERED on each other, which reads as the small word
// "floating" rather than sitting on the same baseline as the big one.
// Shifts the smaller word down by this fraction of the size difference
// to fake a shared baseline instead - an empirical constant (typical
...
// fixed visual pattern allows - line 3 gets a fair third (min 2, since
// it needs at least 1 "small" word + 1 "big" word to show its own
// split), the rest divides between lines 1/2 with any remainder going
// to line 1. Purely mechanical - the model never has to understand or
// mark up the tiering itself, just provide a natural phrase (same
// "don't trust a small model with a fragile rule, enforce it in code"
// precedent as template selection elsewhere in this file).
function splitTextTiersLines(words) {
  const total = words.length;
  const line3Count = Math.max(2, Math.round(total / 3));
  const remaining = total - line3Count;
  const line1Count = Math.max(1, Math.ceil(remaining / 2));
  const line2Count = Math.max(1, remaining - line1Count);
  return [
    words.slice(0, line1Count),
    words.slice(line1Count, line1Count + line2Count),
    words.slice(line1Count + line2Count),
  ];
}

/** Mirrors buildTextTiersLayers' own internal timing constants exactly (same pattern as textPopOutMinDuration) so the two can never drift out of sync - the minimum real screen time this template's build+exit needs for a given word count. */
function textTiersMinDuration(wordCount) {
  const lastWordStart = TEXT_TIERS_WORD_STAGGER * (wordCount - 1);
  const buildEnd = lastWordStart + TEXT_TIERS_POP_DURATION;
  const exitEnd = buildEnd + TEXT_TIERS_MIN_HOLD_AFTER_BUILD + TEXT_TIERS_EXIT_DURATION;
  return exitEnd + TEXT_TIERS_END_BUFFER;
}

function buildTextTiersLayers({ text, accentColor, duration }) {
  const words = text.split(' ').filter((w) => w.length > 0);
  const [line1Words, line2Words, line3Words] = splitTextTiersLines(words);
  ...
  const line3 = line3Words.map((w, i) => ({
    word: w,
  ...
  // ---- layout: per-line horizontal centering, then vertical stacking ----
  const measureCtx = createCanvas(10, 10).getContext('2d');
  function measureWordWidth(word, fontFamily, fontSize) {
  ...
  const lineLayouts = allLines.map((line, lineIdx) => {
    ...
    const startX = [];
    line.forEach((w, i) => {
      if (i > 0) cursor += Math.min(w.fontSize, line[i - 1].fontSize) * TEXT_TIERS_WORD_GAP_RATIO;
      startX.push(cursor);
      cursor += widths[i];
    });
  ...
  const totalBlockHeight = lineLayouts.reduce((sum, l) => sum + l.lineHeight, 0) + TEXT_TIERS_LINE_GAP * 2;
  let cursorY = -totalBlockHeight / 2;
  const lineCenterY = lineLayouts.map((l) => {
    const centerY = cursorY + l.lineHeight / 2;
    cursorY += l.lineHeight + TEXT_TIERS_LINE_GAP;
    return centerY;
  });

  // ---- timeline: build+exit anchored to the beat's own real duration,
  // same "no dead hold before the exit" fix textPopOut already proved.
  const totalWords = words.length;
  const lastWordStart = TEXT_TIERS_WORD_STAGGER * (totalWords - 1);
  const buildEnd = lastWordStart + TEXT_TIERS_POP_DURATION;
  const minExitEnd = buildEnd + TEXT_TIERS_MIN_HOLD_AFTER_BUILD + TEXT_TIERS_EXIT_DURATION;
  const EXIT_END = typeof duration === 'number' && Number.isFinite(duration)
    ? Math.max(duration - TEXT_TIERS_END_BUFFER, minExitEnd)
    : minExitEnd;
  ...
  allLines.forEach((line, lineIdx) => {
    line.forEach((w, wordIdx) => {
      const localX = lineLayouts[lineIdx].localX[wordIdx];
      let localY = lineCenterY[lineIdx];
      if (lineIdx === 2 && w.small) {
        localY += (TEXT_TIERS_BIGGEST_SIZE - w.fontSize) * TEXT_TIERS_BASELINE_COMPENSATION;
      }
...
function buildMographBeatVisual(beat) {
  ...
  } else if (spec.type === 'textTiers' && typeof spec.text === 'string' && spec.text.trim()) {
    const words = spec.text.trim().split(/\s+/).filter((w) => w.length > 0).slice(0, 10);
    if (words.length >= 5) {
      const text = words.join(' ');
      // Same "resolve the final duration BEFORE building layers, then
