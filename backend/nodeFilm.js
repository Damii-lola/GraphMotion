...
const NODE_FLOW = `RELEVANCE IS EVERYTHING. A viewer must know within one second what THIS company is from the pictures, the objects and the words alone. So before anything else you READ THE BRIEF and write "motifs": 6 short, concrete, PHOTOGRAPHABLE things that belong to THIS client's real world, each taken from words or facts in the brief: the product itself, what it is made of or does, the place where it is made or used, the tools of the trade, the customer's moment, the result. A motif is always something a camera could film - never an abstract word, an app name or a brand (not "TikTok" but "a phone playing a vertical video"; not "AI" but "a laptop showing a video editing timeline"; not "waitlist" but "a laptop with a signup screen"). Example for a coffee roaster: green coffee beans, a copper roasting drum, a steaming espresso cup, a burlap sack, a barista's hands pouring, morning light in a cafe window. Example for an app builder that turns typed descriptions into working apps: a laptop with a chat box typing a prompt, a phone showing a finished app, a monitor with colourful code, a desk with sticky notes, a laptop with a publish button on screen, a founder's desk at night. EVERYTHING that follows - every node, every world picture, every headline - must be built from these motifs and the client's own words. Generic decoration that has nothing to do with the client (rockets, planets, stars, galaxies, microphones, globes, trophies, lightbulbs, lightning bolts, brains, robots, diamonds, crowns) is FORBIDDEN unless the brief itself is about it.
...
async function cutout(buf, tol = 70) {
  ...
  if (greenBg) {
    // distance (in pixels) from the removed area, up to 6: the halo lives there
    const dist = new Uint8Array(W * H).fill(255); let q = [];
    for (let p = 0; p < W * H; p++) if (gone[p]) { dist[p] = 0; q.push(p); }
    for (let step = 1; step <= 6 && q.length; step++) {
      const nq = [];
      for (const p of q) { const x = p % W, y = (p / W) | 0; for (const n of [x > 0 ? p - 1 : -1, x < W - 1 ? p + 1 : -1, y > 0 ? p - W : -1, y < H - 1 ? p + W : -1]) { if (n >= 0 && dist[n] === 255) { dist[n] = step; nq.push(n); } } }
      q = nq;
    }
    for (let p = 0; p < W * H; p++) {
      if (gone[p]) continue;
      const r = d[p * 4], g2 = d[p * 4 + 1], b2 = d[p * 4 + 2], cap = Math.max(r, b2), lean = g2 - cap;
      if (dist[p] <= 6 && lean > 14 && g2 > 60) { gone[p] = 1; continue; }                 // a green-tinted halo pixel next to the background: it was never part of the object
      if (lean > 0) d[p * 4 + 1] = cap + lean * 0.1;                                          // everything else just loses its green cast
    }
  }
