...
(1) THE PLACE. Write "place": ONE specific, beautiful, cinematic environment that belongs to THIS client's real world (a coffee brand: a rustic roastery at dawn with a copper drum and burlap sacks; an app builder or AI tool: a modern studio loft at dusk with a wall of big windows and SEVERAL monitors and laptops whose screens glow with a colourful interface; a shoe brand: a misty forest trail at first light; a bank: a calm glass lobby at blue hour), 40-55 words: the room or landscape, its materials, the light, the colour palette, and 4-5 props taken from your motifs standing IN the place. The client's OWN things must be clearly visible in the place: for software or an AI tool that means real screens showing bright, colourful product-like interfaces (never blank grey screens). Never props that have nothing to do with the client (an apple, a cup of coffee, a plant, a book) unless the client sells them. It must look like a real location a film crew would shoot.
(2) SIX SHOTS of that place, one per scene. EVERY shot must clearly show at least one of the client's own things (a screen with the product, the product itself, the tool of the trade); a shot with nothing from the client's world in it is a failure. Each scene gives: "shot" (where the camera is and what it looks at, max 16 words, every shot clearly DIFFERENT from the one before: a wide establishing view, a low angle along the main surface, a view across the space, a detail of one of the client's things in soft focus, a high angle over the whole space, a calm final wide with empty space), "moment" (a small change tied to THIS client's world, max 12 words - for software: a screen fills with a colourful interface, a video starts playing on a monitor, a timeline scrolls; for a coffee brand: fresh beans pour into the hopper; for shoes: light catches the laces; never generic weather or steam), and "motion": ONE sentence (max 24 words) filming the move from THIS shot into the NEXT (a dolly in, an orbit, a crane up, a push through the window, a pull back, a slow tilt) plus what physically happens on the way. The last scene's "motion" is a slow push in. The hook (scene 1) is the most striking, beautifully lit view of the place.
...
function applyPlacePlan(spec, S, briefText) {
  ...
  if ((bad || !nodeFilm.grounded(place, briefText, mStems)) && motifs.length) {
    ...
    const digital = /(^|[^a-z])(app|apps|software|platform|website|online|digital|saas|ai|video|videos|screen|screens|code|tool|tools)([^a-z]|$)/i.test(motifs.join(' ') + ' ' + briefText.slice(0, 900));
    place = digital
      ? 'a modern creative studio loft at dusk with a wall of tall windows, several large monitors and laptops on long desks whose screens glow with bright, colourful interfaces, ' + props + ' in the space, warm golden light against blue evening sky, deep cinematic atmosphere'
      : 'a real, beautifully lit, cinematic interior or landscape from the world of ' + spec.brand + ', with ' + props + ' standing in the space as props, warm golden light, deep atmosphere';
  }
