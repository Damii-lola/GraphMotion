...
const scriptClient = require('./scriptClient');   // optional stronger script writer (GitHub Models, Cerebras, Mistral, Groq...); Cloudflare is the automatic fallback
const MOVIE_FLOW = relevance.MOVIE_FLOW;   // relevance rules + keyframe film (see relevance.js)
const MOVIE_KEYS = relevance.MOVIE_KEYS;
...
function normalizeSpec(raw, brand, opts = {}) {
  ...
  const spec = {
    ...
    imageStyle: relevance.cleanStyle(str(S.imageStyle, '', 160), 'bright natural daylight, vivid colour, warm and clear'),
    theme: {
      look, ...LOOKS[look], bg,
  ...
  // RELEVANCE: keyframe prompts must be built from the client's own motifs; anything else (a Tesla for an AI video tool) is replaced from the motifs
  const motifs = (Array.isArray(S.motifs) ? S.motifs : []).map((m) => String(m || '').replace(/["<>]/g, '').trim().slice(0, 90)).filter(Boolean).slice(0, 6);
  spec.motifs = motifs;
  if (motifs.length) spec.scenes.forEach((sc, k) => {
    if (!relevance.grounded(sc.imagePrompt, briefText, motifs)) { console.warn('[relevance] scene ' + (k + 1) + ' was not about this company: rebuilt from the motifs'); sc.imagePrompt = relevance.motifPrompt(motifs, k, spec.imageStyle); }
  });
