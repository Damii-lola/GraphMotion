...
const IMAGE_SUFFIX = ', bold commercial photograph for a social media ad, ONE clear subject in the centre, vertical composition, vibrant rich colour, crisp cinematic lighting, shallow depth of field, clean uncluttered background, no text, no letters, no logos, no watermark';
...
- Images: each imagePrompt is the LITERAL PICTURE of that scene's headline - what a viewer would see if the headline were a photograph - built from objects, places and materials of THIS company's world (its product, its customer's real moment). If the picture would fit any other company, rewrite it. Use metaphors that carry the meaning (an idea as a glowing bulb, a hand sketching on paper that turns into a glowing interface). A device always shows a glowing, colourful screen of abstract shapes, never a blank or black one. Never name the company and never include writing (signs, labels, notes with words, text on screens): the picture AI garbles lettering. NEVER a group of smiling people, an office team, a handshake or generic stock-photo people. Rich, vibrant colour.
...
"scenes": EXACTLY 6 objects, each: "id", "tag" (a 1-3 word LABEL pill like "POV", "Real talk", "The proof" - a label, not the start of a sentence, never ending in "..."; "" for cta), "headline" (2-7 words, hard limit; | for a line break; wrap the 1-2 key words in *asterisks*), "sub" (optional line, max 9 words, else ""), "sticker" ({"n":"number or word, max 7 chars","l":"label, max 3 words"} for solution/feature/proof only when the brief gives a real number or fact, else null), "imagePrompt" (max 32 words, ONE subject, no text, no logos, no faces or bodies or bare skin, never a group of people - every image must pass a strict family-friendly safety filter), "move" (see 2), "pos" (where this scene's text sits: "mm" middle, "bm" bottom centre, "bl" bottom left, "br" bottom right - pick the spot that leaves the picture's subject clear, and vary it), "tone" ("dark" except at most one), "fx" ({"ripple":0-1,"mist":0-1,"rays":0-1}),
