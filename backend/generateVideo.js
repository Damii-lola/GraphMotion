...
function register(app, { siteVideo }) {
  ...
  async function render(j) {
    try {
      const dir = store.dirFor(j.id);
      j.phase = 'rendering';
      j.render = siteVideo.submit({ dir }, { sceneSeconds: 3.5, fps: 30, preset: 'tiktok', holdStart: 0, holdEnd: 1, captureBudgetSeconds: +process.env.AD_CAPTURE_BUDGET_S || 420 });   // constants: 720x1280, 30 fps, 3.5 s per scene; no dead intro (the hook is on screen from frame 1), 1 s to linger on the CTA card
