 ...
 * ("[break] immediately after every comma-like mark, [break][break]
...
2. Insert [break][break] (two [break] tags back to back, no other text between them) immediately after EVERY sentence-ending mark (. ? ! or ...) in the script - every single one, not just some. There is no separate "long pause" tag - a longer pause is always written as two [break] tags in a row, never anything else.
...
Output: "[curious] Ever wonder why cats knead blankets? [break][break] It's actually a deep instinct, [break] from when they were kittens. [break][break]"`;

/** Deterministic guarantee for the two pause rules - runs regardless of what the model did, so rules 1-2 above are never actually optional. Skips insertion where a [break] is already immediately present (avoids double-tagging a spot the model already got right) - for sentence-enders this means it won't pile a third [break] onto a spot that already has one or two. Ellipsis ("...") is matched as ONE unit, not three separate sentence-enders. */
function ensurePauseTags(text) {
  let out = text.replace(/([,:;])(?!\s*\[break\])/g, '$1 [break]');
  out = out.replace(/(\.{2,}|[.?!])(?!\s*\[break\])/g, '$1 [break][break]');
  return out;
}
