...
def make_clip(job):
    ...
    frames = pipe(
        conditions=[LTXVideoCondition(image=img, frame_index=0)], prompt=str(job.get("prompt", ""))[:600],
...
if not S:                                              # no server secret: just prove the model works
    ...
    t = time.time()
    data = make_clip({"image": base64.b64encode(open("/tmp/t.png", "rb").read()).decode(), "prompt": "the camera slowly pushes in", "frames": 81})
    print("self-test clip ok:", len(data), "bytes in", round(time.time() - t), "s")
    raise SystemExit
